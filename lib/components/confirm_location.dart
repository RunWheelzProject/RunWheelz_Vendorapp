...
class ConfirmLocation extends StatefulWidget {
  bool isVendor;
  bool isCustomer;
  ConfirmLocation({Key? key, this.isVendor = false, this.isCustomer = false}): super(key: key);
...
class ConfirmLocationState extends State<ConfirmLocation> {
  ...
  Widget build(BuildContext context) {
    ...
    final ServiceRequestManager serviceRequestManager = Provider.of<ServiceRequestManager>(context);
    final LocationManager locationManager = Provider.of<LocationManager>(context);
          ...
          Center(
              child: ElevatedButton(
                  onPressed: () {
                    ...
                    if (widget.isVendor) {
                      VendorRegistrationService().vendorRegistrationRequest(vendorManager.vendorDTO).then((response) {
                    ...
                    if (widget.isCustomer) {
                      BreakdownService().customerRequest(serviceRequestManager.serviceRequestDTO).then((response) {
                        var json = jsonDecode(response.body);
                        serviceRequestManager.serviceRequestDTO = ServiceRequestDTO.fromJson(json);
